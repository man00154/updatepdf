...
[data-testid="stSidebar"]{
    background:linear-gradient(180deg,#0a0e1a,#1a2035,#0d1b2a)!important;}
[data-testid="stSidebar"] *{color:#c9d8f0!important;}
.kcard{border-radius:16px;padding:18px 22px;color:#fff;margin-bottom:10px;
    box-shadow:0 6px 24px rgba(0,0,0,.35);transition:transform .2s;}
.kcard:hover{transform:translateY(-3px);}
.kcard h2{font-size:1.9rem;margin:0;font-weight:800;}
.kcard p{margin:4px 0 0;font-size:.82rem;opacity:.82;}
...
.sec-title{font-size:1.2rem;font-weight:700;color:#1e3c72;
    border-left:5px solid #2a5298;padding-left:10px;margin:18px 0 10px;}
/* AI chat bubbles */
.q-user{background:linear-gradient(135deg,#1e3c72,#2a5298);color:#fff;
    border-radius:18px 18px 4px 18px;padding:11px 16px;
    margin:10px 0 4px auto;max-width:76%;width:fit-content;font-size:.97rem;
...
.cell-chip{background:#1a2f1a;border-left:4px solid #27ae60;border-radius:7px;
    padding:7px 12px;margin:4px 0;font-family:monospace;font-size:.8rem;color:#b8ffb8;}
.clearfix{clear:both;}
.warn-box{background:#3a1a00;border-left:4px solid #e67e22;border-radius:8px;
    padding:12px 16px;color:#ffd9a0;font-size:.9rem;margin:6px 0;}
</style>
""", unsafe_allow_html=True)
 
# ──────────────────────────────────────────────────────────────────
# STOP-WORDS for AI query parsing
# ──────────────────────────────────────────────────────────────────
_SW = {
...
# ──────────────────────────────────────────────────────────────────
# PATH RESOLUTION  – works on local machine AND Streamlit Cloud
# ──────────────────────────────────────────────────────────────────
def _app_dir() -> Path:
    """Return the directory containing app.py, robust to all environments."""
    try:
...
def location_from_name(fname: str) -> str:
    n = (fname
         .replace("Customer_and_Capacity_Tracker_", "")
         .replace(".xlsx", "").replace(".xls", ""))
    n = re.sub(
        r"_\d{2}(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\d{2,4}.*$",
        "", n, flags=re.I,
    )
...
def save_uploads(file_bytes_list: tuple) -> str:
    """
    Persist Streamlit UploadedFile objects to a temp dir.
    Accepts a tuple of (name, bytes) pairs so it is hashable for caching.
    """
...
# ──────────────────────────────────────────────────────────────────
# XLS → XLSX  via LibreOffice  (handles true OLE2 .xls files)
# ──────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def ensure_readable(original_path: str) -> str:
    """
    If the file is a real OLE2 .xls, convert it to xlsx via LibreOffice
    and return the new path.  Otherwise return original_path unchanged.
    """
    if not original_path.lower().endswith(".xls"):
        return original_path
    # OLE2 magic bytes check
    try:
    ...
    try:
        ...
        converted = os.path.join(out_dir, basename + ".xlsx")
...
# ──────────────────────────────────────────────────────────────────
# READ ONE EXCEL FILE  (all sheets, raw, dtype=str)
# ──────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def load_file(original_path: str) -> dict:
    """
    Load every sheet as a raw DataFrame (no header parsing, dtype=str).
    Handles both .xlsx (openpyxl) and .xls (LibreOffice conversion).
...
def best_header_row(df: pd.DataFrame) -> int:
    """Pick the row in the first 8 rows that looks most like column headers."""
...
def smart_header(df: pd.DataFrame) -> pd.DataFrame:
    """Return a cleaned DataFrame with unique column names from best header row."""
...
# Handles sheets like Chennai/Vashi that have multiple header zones
# at different positions inside the same sheet.
# ──────────────────────────────────────────────────────────────────
def _detect_all_header_rows(df: pd.DataFrame) -> set:
    """
    Find ALL rows that look like column-header rows throughout the sheet.
    A row qualifies if ≥70% of its filled cells are text labels (not numbers),
    has ≥2 filled cells, and ≥2 distinct values.
    """
    hr_set = set()
...
def _build_cell_col_map(df: pd.DataFrame):
    """
    Returns (cell_map, hr_set).
    cell_map[(row, col)] = best column-header name for that cell.
    Walks each cell top-to-bottom; updates the header whenever a header row is hit.
    This correctly assigns "Customer Name" to customer rows even when the header
    row appears in the middle of the sheet (e.g. row 20 in Chennai).
    """
    hr_set = _detect_all_header_rows(df)
 
    # Build column-name map for each detected header row
    hr_maps = {}
    for hr in hr_set:
        m = {}
...
# ──────────────────────────────────────────────────────────────────
# BUILD FULL CORPUS
# Every non-empty cell of every row × col × sheet × file.
# No skips, no conditions, no row/column restrictions.
# ──────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def build_corpus(file_list: tuple, folder: str):
    ...
    Parameters
    ----------
    file_list : tuple of file names (must be hashable for st.cache_data)
    folder    : directory that contains those files
 
    Returns
    -------
    corpus      : list[dict]  – one entry per non-empty cell
    row_records : dict        – (fname,loc,sheet,row) → {col_header: value}
    meta        : dict        – stats / location list
    """
    corpus      = []
    row_records = defaultdict(dict)
 
    for fname in file_list:
        full_original = os.path.join(folder, fname)
        if not os.path.isfile(full_original):
            continue                      # file truly does not exist – skip silently
 
        path = ensure_readable(full_original)
        ...
        for sh in xf.sheet_names:
            ...
            # Per-cell column-name map (handles multi-section sheets)
...
# ──────────────────────────────────────────────────────────────────
st.sidebar.image(
    "https://img.icons8.com/fluency/96/data-center.png", width=72
)
...
if uploaded_files:
    # Convert to hashable (name, bytes) pairs for caching
...
if not excel_files:
    st.error(
        ...
        "**How to fix:**\n\n"
        "1. **Upload files** using the sidebar uploader above, **OR**\n"
        "2. Create an `excel_files/` folder next to `app.py` and place "
        "all 10 Excel files there, then redeploy.\n\n"
...
with tabs[0]:
    ...
    ci = pd.DataFrame({
        ...
        "Sample":   [str(df_clean[c].dropna().iloc[0])[:60]
...
with tabs[1]:
    ...
    st.download_button("⬇️ Download CSV",
                       ...
                       "export.csv", "text/csv")
    st.markdown("---")
    st.subheader("🗃️ Raw Excel View (no header processing)")
...
with tabs[2]:
    ...
    if not num_cols:
        ...
        all_cat = [c for c in df_clean.columns
    ...
    if not num_cols:
        ...
        def _s(label, opts, idx=0, key=None):
            return st.selectbox(label, opts,
                                index=min(idx, max(0, len(opts)-1)), key=key)
        ...
        if ctype == "Bar Chart":
            ...
            if len(num_cols) >= 3:
                xc = _s("X", num_cols, 0, "bu_x"); yc = _s("Y", num_cols, 1, "bu_y")
                sz = _s("Size", num_cols, 2, "bu_s"); lc = _s("Color", ["None"]+cat_cols, key="bu_c")
            ...
            if len(num_cols) >= 3:
                xc = _s("X", num_cols, 0, "3x"); yc = _s("Y", num_cols, 1, "3y")
                zc = _s("Z", num_cols, 2, "3z"); cc = _s("Color", ["None"]+cat_cols, key="3c")
...
# ═══════════════════════════════════════════════════════
with tabs[4]:
    st.subheader("🥧 Distribution & Composition Charts")
    if not num_cols:
        ...
        with r1:
            ...
            pie_cats = [c for c in df_clean.columns
                        if c not in num_cols and 1 < df_clean[c].nunique() <= 30]
            if pie_cats:
                pc  = st.selectbox("Category", pie_cats, key="pcat")
        ...
        tm_cats = [c for c in df_clean.columns
                   if c not in num_cols and 1 < df_clean[c].nunique() <= 50]
        if tm_cats and num_cols:
            tmc = st.selectbox("Category", tm_cats, key="tmc")
            tmv = st.selectbox("Value",    num_cols, key="tmv")
            tmd = (df_clean[[tmc,tmv]].dropna()
                                      .groupby(tmc)[tmv].sum().reset_index())
            tmd = tmd[tmd[tmv] > 0]
            if len(tmd):
        ...
        sun_cats = [c for c in df_clean.columns
                    if c not in num_cols and 1 < df_clean[c].nunique() <= 40]
...
with tabs[5]:
    ...
    st.info("For cross-file search across ALL sheets use the **💬 AI Smart Query** tab.")
        ...
        if any(w in ql for w in ["customer","list","show"]):
            for c in df.columns:
                if "customer" in c.lower() or "name" in c.lower():
                    ...
                    break
        if not res:
            res.append("ℹ️ Try: **sum / average / max / min / count / median / std / unique / missing / list**")
        return "\n\n".join(res)
    ...
    if num_cols:
        ...
        if op == "Cumulative Sum" and len(s):
            st.plotly_chart(px.line(s.cumsum().reset_index(drop=True),
                                    title=f"Cumulative: {sc}", markers=True),
                            use_container_width=True)
...
with tabs[6]:
    ...
    if all_summ:
        ...
        if rows:
            ...
            st.plotly_chart(fig, use_container_width=True)
            fig2 = px.scatter(cmp.reset_index(), x="Mean", y="Max",
                              size="Sum", text="Location|Sheet", color="Count",
                              color_continuous_scale="Turbo", title="Bubble: Mean vs Max")
            fig2.update_traces(textposition="top center"); fig2.update_layout(height=480)
            ...
            th  = nrm.index.tolist(); rv = nrm.values.tolist()
        ...
        with st.spinner("Analysing all sheets…"):
            for lbl, info in list(all_summ.items())[:10]:
                ...
                with st.expander(f"📍 {lbl}", expanded=False):
                    ...
                    with cb:
                        st.markdown("**⚠️ Anomalies (Z-score > 2.5)**")
                        for col in nc[:4]:
                            s = dfa[col].dropna()
                            if len(s) > 3:
                                z = (s - s.mean()) / s.std()
                    ...
                    if len(nc) >= 2:
                        fig = px.bar(dfa[nc[:3]].dropna().reset_index(),
                                     x="index", y=nc[0], color_discrete_sequence=["#2a5298"],
                                     title=nc[0])
                        fig.update_layout(height=240, showlegend=False, margin=dict(t=28,b=0))
                        st.plotly_chart(fig, use_container_width=True)
                    if len(nc) >= 2:
                        cs = (dfa[nc].corr().unstack().drop_duplicates())
                        cs = cs[cs.index.get_level_values(0) != cs.index.get_level_values(1)
                               ].abs().sort_values(ascending=False)
                        if len(cs):
                            p = cs.index[0]
                            st.info(f"🔗 Top corr: **{p[0]}** ↔ **{p[1]}** ({cs.iloc[0]:.3f})")
...
# Searches EVERY cell – any row, any col, any position in any sheet.
# Works for unstructured/odd Excel layouts. Zero formulas.
# ═══════════════════════════════════════════════════════════════════
with tabs[8]:
 
    # ── Build corpus with progress feedback ─────────────────────────
    corpus_status = st.empty()
    corpus_status.info("🔍 Indexing every cell in every file…")
    corpus, row_records, meta = build_corpus(tuple(excel_files), data_dir)
    corpus_status.empty()
 
    # ── Warn if corpus is empty ──────────────────────────────────────
    if not corpus:
        st.markdown(
            '<div class="warn-box">⚠️ <b>Corpus is empty.</b> No data was indexed. '
            'Make sure Excel files exist in the <code>excel_files/</code> folder '
            'or upload them via the sidebar.</div>',
            unsafe_allow_html=True,
        )
        st.stop()
    ...
    st.markdown(
        "Type **any question** in plain English. The engine reads "
        "**every row · every column · every sheet · every file** "
        "— including data in the middle of unstructured sheets — "
    ...
    with st.expander("🔧 Optional: Narrow scope before asking", expanded=False):
        scope_locs   = st.multiselect(
            "Limit to locations (blank = ALL)", meta["locations"], default=[],
            key="scope_locs",
        )
        scope_sheets = st.multiselect(
            "Limit to sheet names (blank = ALL)",
    ...
    def ai_smart_query(question: str) -> dict:
        ...
        Scans the pre-built corpus (every non-empty cell).
        No Excel formulas. No hard-coded row/column conditions.
        """
        q   = question.strip()
        ql  = q.lower()
        ...
        out = {
            "answer": "", "table": None,
            "chart_df": None, "chart_cfg": None,
            "cell_hits": [], "sub_tables": [],
        }
 
        # ── Apply scope filters ──────────────────────────────────────
        # Start with full corpus, then narrow
        wc = list(corpus)
        if scope_locs:
        ...
        if not wc:
            out["answer"] = "⚠️ No data in the selected scope."
            return out
 
        # ── Auto-detect location filter from question ────────────────
        loc_filt = None
        ...
        if loc_filt:
            wc = [c for c in wc if loc_filt.lower() in c["location"].lower()]
 
        if not wc:
            out["answer"] = (
                f"❓ No data found for location **'{loc_filt}'**.\n\n"
        ...
        _OP_WORDS = {
            ...
            "proportion","number","across","compare","location","locations",
        }
 
        def best_col_kw(slist):
            cands = [w for w in slist if w not in _OP_WORDS and len(w) >= 3]
            best, bn = None, 0
            for w in cands:
        ...
        def num_for_kw(kw):
            """Numeric values from cells whose col_header contains kw – ANY row/col."""
            res = []
            for cell in wc:
                if cell["is_header"]:
                    continue
        ...
        if f_shts and not f_num:
            ...
            out["answer"] = f"Found **{len(tbl)}** sheet(s){' in '+loc_filt if loc_filt else ''}."
            out["table"]  = tbl
            return out
        ...
        if f_miss:
            ...
            for fname in excel_files:
                loc = location_from_name(fname)
                if loc_filt and loc_filt.lower() not in loc.lower():
                    continue
                    ...
                    for col in dfc.columns:
                        ...
                        if mc > 0:
                            mrows.append({"Location":loc,"Sheet":sh,"Column":col,
                                          "Missing":mc,"Missing%":f"{mc/max(len(dfc),1)*100:.1f}%"})
            if mrows:
        ...
        if f_cols and not f_num:
            ...
            for cell in wc:
                ...
                if k not in seen:
                    seen.add(k)
                    crows.append({"Location":cell["location"],"Sheet":cell["sheet"],"Column":ch})
        ...
        if f_rows and f_cnt and not sig:
            ...
            for fname in excel_files:
                loc = location_from_name(fname)
                if loc_filt and loc_filt.lower() not in loc.lower(): continue
                shd = load_file(os.path.join(data_dir, fname))
                for sh, raw in shd.items():
                    if scope_sheets and sh not in scope_sheets: continue
                    dfc = smart_header(raw)
                    crows.append({"Location":loc,"Sheet":sh,"Rows":len(dfc)}); total+=len(dfc)
            tbl = pd.DataFrame(crows)
            out["answer"] = f"**{total:,}** total data rows across **{len(tbl)}** sheet(s)."
            out["table"]  = tbl
        ...
        if f_num and sig:
            ...
            if kw:
                ...
                if pairs:
                    ...
                    if f_stat:
                        parts.append(
                            f"**Median:**  {s_all.median():,.4f}\n"
                            f"**Std Dev:** {s_all.std():,.4f}")
                    if f_pct:
                        grand = sum(float(c["value"]) for c in wc
                                    if not c["is_header"] and _try_float(c["value"]))
                    ...
                    out["answer"] = (
                        f"Results for columns matching **'{kw}'**  "
                    ...
                    if f_botn:
                        ...
                        out["sub_tables"].append({
                            "label": f"🔻 Bottom {n} values for '{kw}'",
                            "df": pd.DataFrame([{
                                "📍 Location":c["location"],"📋 Sheet":c["sheet"],
                                "Row #":c["row"]+1,"Col #":c["col"]+1,
                                "Column":c["col_header"],"Value":v
        ...
        if f_uniq and sig:
            ...
            if kw:
                ...
                out["answer"] = (
                    f"Found **{len(uvals)}** unique value(s) in columns matching "
                    f"**'{kw}'**{' in '+loc_filt if loc_filt else ''}."
                )
                out["table"] = tbl
                return out
 
        # ── INTENT: Free-text entity / keyword search ─────────────────
        # Searches VALUES in EVERY cell at ANY row, ANY column, ANYWHERE.
        if sig:
            quoted = re.findall(r'"([^"]+)"', q)
            if quoted:
                terms = [quoted[0].lower()]
            else:
                # Only keep words that actually appear in cell values
                terms = [w for w in sig
                         if any(w in cell["value"].lower() for cell in wc)]
                if not terms:
                    terms = sig    # fallback: use all sig words
            ...
            if not full_df.empty:
                for col in full_df.columns:
                    if "customer" in col.lower() or "name" in col.lower():
                        cust_df = (full_df[["📍 Location","📋 Sheet",col]]
                                     .drop_duplicates())
                        out["sub_tables"].append({
            ...
            if num_hits and f_topn:
                ...
                out["sub_tables"].append({
                    "label": f"🏆 Top {n} numeric values in results",
                    "df": pd.DataFrame([{
                        "Location":c["location"],"Sheet":c["sheet"],
        ...
        out["answer"] = (
            ...
            "• Numeric:   *Total subscription*  |  *Max capacity Airoli*  "
                          "|  *Average power usage*  |  *Top 10 subscription*\n"
            "• List:      *List all customers*  |  *Customers in Noida*\n"
            "• Unique:    *Unique billing models*  |  *Distinct floors*\n"
            "• Meta:      *Show all sheets*  |  *List columns*  "
        ...
        if res.get("chart_cfg") and res.get("chart_df") is not None:
            ...
            if cfg["x"] in cdf.columns and cfg["y"] in cdf.columns:
                fig = px.bar(
                    ...
                    x=cfg["x"], y=cfg["y"],
                    color=cfg["y"], color_continuous_scale="Viridis",
    ...
    with ic:
        user_q = st.text_input(
            "Ask:",
            placeholder=(
                "Find CISCO  |  List all customers in Noida  |  "
                "Total subscription Airoli  |  Max capacity  |  "
                "Average power usage  |  Top 10 subscription"
            ),
...
st.markdown("---")
st.caption(
    "Sify DC · Customer & Capacity Tracker · "
    "Reads every cell of every sheet · "
    f"Data dir: `{data_dir}`"
)
