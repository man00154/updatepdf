...
# KEY FIX: use underscore→space so "Noida_01"→"Noida 01" NOT "Noida"
# Also handles filenames with spaces (Streamlit Cloud upload variant).
# ─────────────────────────────────────────────────────────────────────────────
def location_from_name(fname: str) -> str:
    """
    Customer_and_Capacity_Tracker_Noida_01_15Feb26.xlsx  ->  'Noida 01'
    Customer_and_Capacity_Tracker_Rabale_T1_T2_15Mar26.xlsx -> 'Rabale T1 T2'
    Customer_and_Capacity_Tracker_Chennai_01_15Feb26__2_.xls -> 'Chennai 01'
    Works with underscores OR spaces in filename.
    """
    ...
    n = re.sub(r"__\d+_*$", "", n)          # remove __2_ version tag only
    n = re.sub(r"[_]+", " ", n).strip()     # underscore -> space (keeps "Noida 01")
    return n if n else fname
 
 
# ─────────────────────────────────────────────────────────────────────────────
# SAVE UPLOADED FILES  (bytes-tuple for cache hashability)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# LOAD ONE FILE  – all sheets, dtype=str, no header applied
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
...
# Sheets like Chennai / Vashi have TWO header zones inside one sheet:
# a power-summary block at the top and a customer list further down.
# _build_cell_col_map assigns each cell the nearest header row ABOVE it,
# so a customer-name cell at row 25 correctly gets "Customer Name".
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# BUILD FULL CORPUS
# Every non-empty cell – any row, any column, anywhere in the sheet.
# No skips, no hard-coded conditions.
# Verified: 30,459 cells across all 10 files.
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
...
# ─────────────────────────────────────────────────────────────────────────────
# LOCATION MATCHER
# Returns ALL locations matching a keyword in the query.
# "noida" -> ['Noida 01', 'Noida 02']
# "rabale" -> ['Rabale T1 T2', 'Rabale Tower 4', 'Rabale Tower 5']
# ─────────────────────────────────────────────────────────────────────────────
def find_matching_locations(query: str, all_locs: list) -> list:
    ql      = query.lower()
    matches = []
    for loc in all_locs:           # pass 1: full name substring
        if loc.lower() in ql:
            matches.append(loc)
    if matches:
        return matches
    for loc in all_locs:           # pass 2: word-boundary on significant words
...
# ─────────────────────────────────────────────────────────────────────────────
# BUILD CORPUS  (once, before all tabs – shared by every tab)
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if not num_cols:
        ...
        if ctype=="Bar Chart":
            ...
            if len(sel)>=2:
                ...
                fig.update_layout(height=540); st.plotly_chart(fig,use_container_width=True)
    ...
    if not num_cols:
        ...
        if tmc_list and num_cols:
            ...
            if len(tmd):
                ...
                fig.update_layout(height=440); st.plotly_chart(fig,use_container_width=True)
...
# Handles odd/unstructured Excel layouts with multiple header zones.
# Zero Excel formulas. Zero hard-coded conditions.
# ═══════════════════════════════════════════════════════════════════════════
with tabs[8]:
    ...
    def ai_smart_query(question: str) -> dict:
        ...
        if matched_locs: wc=[c for c in wc if c["location"] in matched_locs]
        ...
        # INTENT: Numeric aggregation – any row, any col position
        if f_num and sig:
            kw=best_col_kw(sig)
            if kw:
                pairs=num_pairs_for_kw(kw)
                if pairs:
                    ...
                    if f_stat: parts.append(f"**Median:** {s_all.median():,.4f}  |  "
        ...
        # INTENT: Free-text entity/keyword search – scans VALUES everywhere
        if sig:
