import os, re, io, warnings, tempfile, subprocess
...
# ── FIXED: works with BOTH underscore AND space-separated filenames ─
def location_from_name(fname: str) -> str:
    """
    Extract a short human-readable location name from ANY filename variant.
    Handles: underscores, spaces, mixed case, double underscores (__2_), etc.
    Examples:
      Customer_and_Capacity_Tracker_Noida_01_15Feb26.xlsx  → 'Noida 01'
      Customer and Capacity Tracker Bangalore 01 15Feb26.xlsx → 'Bangalore 01'
    """
    # Take basename only
    n = os.path.basename(fname)
    # Remove extension
    n = re.sub(r"\.(xlsx?|xls)$", "", n, flags=re.I)
    # Remove the tracker prefix (underscores OR spaces, any case)
    n = re.sub(
        r"[Cc]ustomer.?[Aa]nd.?[Cc]apacity.?[Tt]racker.?",
        "", n,
    )
    # Remove trailing date patterns like _15Mar26, _01_15Feb26, 15Feb26, __2_
    n = re.sub(
        r"[_\s]?\d{2}(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\d{2,4}.*$",
        "", n, flags=re.I,
    )
    # Remove trailing __2_ style version suffixes (double-underscore + digit)
    n = re.sub(r"__\d+_*$", "", n)
    # Normalise separators to single space
    n = re.sub(r"[_\s]+", " ", n).strip()
    return n if n else fname  # fallback to full name if something went wrong
 
 
# ═══════════════════════════════════════════════════════════════════
#  SAVE UPLOADED FILES  (hashable for st.cache_data)
# ═══════════════════════════════════════════════════════════════════
@st.cache_data(show_spinner=False)
def save_uploads(file_bytes_tuple: tuple) -> str:
    """Persist (name, bytes) pairs to a temp dir. Returns the dir path."""
...
# ═══════════════════════════════════════════════════════════════════
#  XLS → XLSX  (LibreOffice, OLE2 detection)
# ═══════════════════════════════════════════════════════════════════
@st.cache_data(show_spinner=False)
...
# ═══════════════════════════════════════════════════════════════════
#  LOAD ONE FILE  (all sheets, raw dtype=str)
# ═══════════════════════════════════════════════════════════════════
@st.cache_data(show_spinner=False)
...
#  Handles sheets with multiple header zones (Chennai, Vashi, etc.)
# ═══════════════════════════════════════════════════════════════════
...
if not excel_files:
    st.error(
        ...
        "place all 10 Excel files there, then redeploy. "
...
st.sidebar.markdown("---")
st.sidebar.caption(
    f"📊 {len(num_cols)} numeric · {len(df_clean)} rows · "
    f"{len(excel_files)} file(s)"
)
...
with tabs[0]:
    ...
    c5.markdown(f'<div class="kcard kcard-teal"><h2>{raw_df.shape[1]}</h2><p>Raw Cols</p></div>',unsafe_allow_html=True)
    ...
    ci = pd.DataFrame({
        ...
        "Sample":   [str(df_clean[c].dropna().iloc[0])[:55] if df_clean[c].dropna().shape[0]>0 else "—" for c in df_clean.columns],
    })
...
with tabs[2]:
    ...
    if not num_cols:
        ...
        if all_cat and num_cols:
            ...
            gc=gc1.selectbox("Group by",all_cat); ac=gc2.selectbox("Aggregate",num_cols); af=gc3.selectbox("Function",["sum","mean","count","min","max","median"])
            grp=df_clean.groupby(gc)[ac].agg(af).reset_index().rename(columns={ac:f"{af}({ac})"}).sort_values(f"{af}({ac})",ascending=False)
            st.dataframe(grp,use_container_width=True)
            fig=px.bar(grp,x=gc,y=f"{af}({ac})",color=f"{af}({ac})",color_continuous_scale="Viridis",title=f"{af.title()} of {ac} by {gc}")
            fig.update_layout(xaxis_tickangle=-35,height=400); st.plotly_chart(fig,use_container_width=True)
...
with tabs[3]:
    ...
    if not num_cols:
        ...
        if ctype=="Bar Chart":
            xc=_s("X",cat_cols or df_clean.columns.tolist(),key="bx"); yc=_s("Y",num_cols,key="by")
            ori=st.radio("Orientation",["Vertical","Horizontal"],horizontal=True)
            d=df_clean[[xc,yc]].dropna()
            fig=px.bar(d,x=xc if ori=="Vertical" else yc,y=yc if ori=="Vertical" else xc,color=yc,color_continuous_scale="Turbo",orientation="v" if ori=="Vertical" else "h",title=f"{yc} by {xc}")
            ...
            if ycs:
                fig=px.bar(df_clean[[xc]+ycs].dropna(subset=ycs,how="all"),x=xc,y=ycs,barmode="group"); fig.update_layout(height=460); st.plotly_chart(fig,use_container_width=True)
        elif ctype=="Line Chart":
            xc=_s("X",df_clean.columns.tolist(),key="lx"); ycs=st.multiselect("Y",num_cols,default=num_cols[:2])
            if ycs:
                fig=px.line(df_clean[[xc]+ycs].dropna(subset=ycs,how="all"),x=xc,y=ycs,markers=True); fig.update_layout(height=450); st.plotly_chart(fig,use_container_width=True)
            ...
            fig=px.scatter(d,x=xc,y=yc,size=sc if sc!="None" else None,color=cc if cc!="None" else None,color_continuous_scale="Rainbow"); fig.update_layout(height=480); st.plotly_chart(fig,use_container_width=True)
        elif ctype=="Area Chart":
            xc=_s("X",df_clean.columns.tolist(),key="ax"); ycs=st.multiselect("Y",num_cols,default=num_cols[:3])
            if ycs:
                fig=px.area(df_clean[[xc]+ycs].dropna(subset=ycs,how="all"),x=xc,y=ycs); fig.update_layout(height=450); st.plotly_chart(fig,use_container_width=True)
        elif ctype=="Bubble Chart":
            if len(num_cols)>=3:
                xc=_s("X",num_cols,0,"bu_x"); yc=_s("Y",num_cols,1,"bu_y"); sz=_s("Size",num_cols,2,"bu_s"); lc=_s("Color",["None"]+cat_cols,key="bu_c")
                d=df_clean[[xc,yc,sz]].dropna()
                if lc!="None": d[lc]=df_clean[lc]
                fig=px.scatter(d,x=xc,y=yc,size=sz,color=lc if lc!="None" else None,size_max=65,color_discrete_sequence=px.colors.qualitative.Vivid); fig.update_layout(height=500); st.plotly_chart(fig,use_container_width=True)
            else: st.info("Need ≥ 3 numeric columns.")
        elif ctype=="Heatmap (Correlation)":
            sel=st.multiselect("Columns",num_cols,default=num_cols[:12])
            if len(sel)>=2:
                fig=px.imshow(df_clean[sel].corr(),text_auto=".2f",color_continuous_scale="RdBu_r",aspect="auto"); fig.update_layout(height=540); st.plotly_chart(fig,use_container_width=True)
        elif ctype=="Box Plot":
            yc=_s("Value",num_cols,key="bp_v"); xc=_s("Group",["None"]+cat_cols,key="bp_g")
            d=df_clean[[yc]+([xc] if xc!="None" else [])].dropna(subset=[yc])
            fig=px.box(d,y=yc,x=xc if xc!="None" else None,color=xc if xc!="None" else None,points="outliers",color_discrete_sequence=px.colors.qualitative.Pastel); fig.update_layout(height=450); st.plotly_chart(fig,use_container_width=True)
            ...
            fig=px.funnel(d,x=yc,y=xc); fig.update_layout(height=450); st.plotly_chart(fig,use_container_width=True)
            ...
            fig.add_trace(go.Scatter(name="Cumulative",x=cum.index,y=cum,line=dict(color="#f7971e",width=2.5),mode="lines+markers"))
            fig.update_layout(title=f"Cumulative: {yc}",height=450,barmode="group"); st.plotly_chart(fig,use_container_width=True)
        elif ctype=="3-D Scatter":
            if len(num_cols)>=3:
                xc=_s("X",num_cols,0,"3x"); yc=_s("Y",num_cols,1,"3y"); zc=_s("Z",num_cols,2,"3z"); cc=_s("Color",["None"]+cat_cols,key="3c")
...
with tabs[4]:
    ...
    if not num_cols:
        ...
        if tmc_list and num_cols:
            tmc=st.selectbox("Category",tmc_list,key="tmc"); tmv=st.selectbox("Value",num_cols,key="tmv")
            tmd=df_clean[[tmc,tmv]].dropna().groupby(tmc)[tmv].sum().reset_index(); tmd=tmd[tmd[tmv]>0]
            if len(tmd):
                fig=px.treemap(tmd,path=[tmc],values=tmv,color=tmv,color_continuous_scale="Turbo"); fig.update_layout(height=440); st.plotly_chart(fig,use_container_width=True)
        st.markdown('<div class="sec-title">🎻 Violin</div>',unsafe_allow_html=True)
        vc=st.selectbox("Column",num_cols,key="vc")
        fig=px.violin(df_clean[vc].dropna(),y=vc,box=True,points="outliers",color_discrete_sequence=["#c0392b"]); st.plotly_chart(fig,use_container_width=True)
        ...
        if len(sun_cats)>=2 and num_cols:
            ...
            sc1=s1.selectbox("Level 1",sun_cats,key="sc1"); sc2=s2.selectbox("Level 2",[c for c in sun_cats if c!=sc1],key="sc2"); sv=s3.selectbox("Value",num_cols,key="sv")
            sd=df_clean[[sc1,sc2,sv]].dropna(); sd[sv]=pd.to_numeric(sd[sv],errors="coerce"); sd=sd.dropna()
            if len(sd):
                fig=px.sunburst(sd,path=[sc1,sc2],values=sv,color=sv,color_continuous_scale="RdYlGn"); fig.update_layout(height=480); st.plotly_chart(fig,use_container_width=True)
...
with tabs[5]:
    ...
    def run_query(q,df,nc):
        ...
        if "median" in ql: [res.append(f"**MEDIAN `{c}`** = {df[c].median():,.4f}") for c in nc if c.lower() in ql or len(nc)==1]
        if any(w in ql for w in ["describe","statistics","summary"]): res.append("```\n"+df[nc].describe().to_string()+"\n```")
        ...
        if not res: res.append("ℹ️ Try: **sum / average / max / min / count / median / unique / missing / list**")
        return "\n\n".join(res)
    ...
    if num_cols:
        ...
        op=mc1.selectbox("Op",["Sum","Mean","Max","Min","Count","Median","Std Dev","Variance","% of Total","Range","IQR"])
        sc=mc2.selectbox("Column",num_cols); fc=mc3.selectbox("Filter by",["None"]+[c for c in df_clean.columns if c not in num_cols])
        fv=None
        if fc!="None": fv=st.selectbox("Filter value",df_clean[fc].dropna().unique().tolist())
        ds=df_clean.copy()
        if fc!="None" and fv is not None: ds=ds[ds[fc]==fv]
        s=ds[sc].dropna()
        ops={"Sum":s.sum(),"Mean":s.mean(),"Max":s.max(),"Min":s.min(),"Count":s.count(),"Median":s.median(),"Std Dev":s.std(),"Variance":s.var(),"% of Total":f"{s.sum()/max(df_clean[sc].sum(),1)*100:.2f}%","Range":s.max()-s.min(),"IQR":s.quantile(.75)-s.quantile(.25)}
        r=ops.get(op,"N/A")
...
with tabs[6]:
    ...
    if all_summ:
        ...
        if rows:
            ...
            fig=px.bar(cmp.reset_index(),x="Location|Sheet",y="Sum",color="Sum",color_continuous_scale="Viridis",title=f"Sum of '{comp_col}' across locations")
            fig.update_layout(xaxis_tickangle=-30,height=440); st.plotly_chart(fig,use_container_width=True)
            fig2=px.scatter(cmp.reset_index(),x="Mean",y="Max",size="Sum",text="Location|Sheet",color="Count",color_continuous_scale="Turbo",title="Bubble: Mean vs Max")
            fig2.update_traces(textposition="top center"); fig2.update_layout(height=480); st.plotly_chart(fig2,use_container_width=True)
            st.markdown('<div class="sec-title">🕸️ Radar Chart</div>',unsafe_allow_html=True)
            rm=st.radio("Metric",["Sum","Mean","Max"],horizontal=True)
            nrm=cmp[rm]/cmp[rm].max(); th=nrm.index.tolist(); rv=nrm.values.tolist()
            fig3=go.Figure(go.Scatterpolar(r=rv+[rv[0]],theta=th+[th[0]],fill="toself",fillcolor="rgba(42,82,152,.25)",line=dict(color="#2a5298",width=2)))
            fig3.update_layout(polar=dict(radialaxis=dict(visible=True,range=[0,1])),height=500); st.plotly_chart(fig3,use_container_width=True)
...
with tabs[7]:
        ...
        with st.spinner("Analysing all sheets…"):
            for lbl,info in list(all_summ.items())[:10]:
                ...
                with st.expander(f"📍 {lbl}",expanded=False):
                    ...
                    if len(nc)>=2:
                        fig=px.bar(dfa[nc[:3]].dropna().reset_index(),x="index",y=nc[0],color_discrete_sequence=["#2a5298"],title=nc[0])
                        fig.update_layout(height=240,showlegend=False,margin=dict(t=28,b=0)); st.plotly_chart(fig,use_container_width=True)
    st.markdown("---")
    ...
    for f in excel_files:
        ...
        fsm.append({"File":loc_map[f],"Sheets":len(shd),"Rows":sum(len(s) for s in shd.values()),"Cols":sum(len(s.columns) for s in shd.values())})
    fs_df=pd.DataFrame(fsm)
    st.dataframe(fs_df,use_container_width=True)
    fig=px.bar(fs_df,x="File",y="Rows",color="Sheets",color_continuous_scale="Blues",title="Rows per location")
    fig.update_layout(xaxis_tickangle=-30,height=360); st.plotly_chart(fig,use_container_width=True)
...
with tabs[8]:
    ...
    # Build corpus
    status_ph = st.empty()
    status_ph.info("🔍 Indexing every cell in every file…")
    corpus, row_records, meta = build_corpus(tuple(excel_files), data_dir)
    status_ph.empty()
 
    if not corpus:
        st.error(
            "⚠️ **Corpus is empty** — no data was indexed.\n\n"
            "Make sure Excel files exist in `excel_files/` next to `app.py`, "
    ...
    st.markdown(
        ...
        "and returns a direct answer. **No formulas. No conditions.**"
    )
    ...
    # Scope filters
    with st.expander("🔧 Optional: Narrow scope",expanded=False):
        scope_locs   = st.multiselect("Limit to locations (blank = ALL)", meta["locations"], default=[], key="slocs")
        scope_sheets = st.multiselect("Limit to sheets (blank = ALL)", sorted({x["sheet"] for x in corpus}), default=[], key="ssheets")
 
    # ── FIXED: location detection ──────────────────────────────────
    def find_matching_locations(query: str) -> list:
        """
        Return ALL location names that match a location keyword in the query.
        E.g. 'noida' → ['Noida 01', 'Noida 02']
        Returns empty list if no location keyword found.
        """
        ql      = query.lower()
        locs    = meta["locations"]
        matches = []
        # Pass 1: full location name substring match (most specific)
        for loc in locs:
            if loc.lower() in ql:
                matches.append(loc)
        if matches:
            return matches
        # Pass 2: word-boundary match of significant words in location name
        for loc in locs:
            for part in re.findall(r"[a-z]+", loc.lower()):
                if len(part) >= 4 and re.search(r"\b" + re.escape(part) + r"\b", ql):
    ...
    def ai_smart_query(question: str) -> dict:
        ...
        matched_locs = find_matching_locations(q)
        if matched_locs:
            wc = [c for c in wc if c["location"] in matched_locs]
 
        if not wc:
            avail = ", ".join(meta["locations"])
            out["answer"] = (
                f"❓ No data found for the location(s) you mentioned.\n\n"
                f"**Available locations:** {avail}\n\n"
                f"Try: *List all customers in Noida* or *List all customers in Airoli*"
            )
            return out
 
        # ── Operator exclusions for col keyword detection ────────────
        _OP = {
            "total","sum","avg","mean","max","min","count","list","find","show",
        ...
        def best_col_kw(slist):
            cands = [w for w in slist if w not in _OP and len(w) >= 3]
            best, bn = None, 0
            for w in cands:
        ...
        if f_shts and not f_num:
            ...
            out["answer"]=f"Found **{len(tbl)}** sheet(s){loc_str}."; out["table"]=tbl; return out
        ...
        if f_miss:
            ...
            for fname in excel_files:
                loc=location_from_name(fname)
                if matched_locs and loc not in matched_locs: continue
        ...
        if f_rows and f_cnt and not sig:
            ...
            for fname in excel_files:
                ...
                for sh,raw in shd.items():
                    if scope_sheets and sh not in scope_sheets: continue
                    dfc=smart_header(raw); crows.append({"Location":loc,"Sheet":sh,"Rows":len(dfc)}); total+=len(dfc)
            ...
            out["chart_cfg"]={"x":"Location","y":"Rows","title":"Data Rows per Location"}; return out
        ...
        if f_num and sig:
            ...
            if kw:
                ...
                if pairs:
                    ...
                    if f_stat: parts.append(f"**Median:**  {s_all.median():,.4f}  |  **Std Dev:** {s_all.std():,.4f}")
                    if f_pct:
                    ...
                    for lbl2,vs in grp.items():
                        sv=pd.Series(vs); breakdown.append({"Location | Sheet":lbl2,"Count":sv.count(),"Sum":round(sv.sum(),4),"Mean":round(sv.mean(),4),"Max":round(sv.max(),4),"Min":round(sv.min(),4)})
                    tbl=pd.DataFrame(breakdown).sort_values("Sum",ascending=False)
                    loc_str=" in "+", ".join(matched_locs) if matched_locs else ""
                    out["answer"]=(f"Results for columns matching **'{kw}'** ({len(vals):,} values{loc_str}):\n\n"+"\n".join(parts))
                    out["table"]=tbl; out["chart_df"]=tbl; out["chart_cfg"]={"x":"Location | Sheet","y":"Sum","title":f"Sum of '{kw}' by Location/Sheet"}
                    if f_topn:
                        n=int(f_topn.group(1)); top=sorted(pairs,key=lambda x:x[0],reverse=True)[:n]
                        out["sub_tables"].append({"label":f"🏆 Top {n} values for '{kw}'","df":pd.DataFrame([{"📍 Location":c["location"],"📋 Sheet":c["sheet"],"Row #":c["row"]+1,"Col #":c["col"]+1,"Column":c["col_header"],"Value":v} for v,c in top])})
                    if f_botn:
                        n=int(f_botn.group(1)); bot=sorted(pairs,key=lambda x:x[0])[:n]
                        out["sub_tables"].append({"label":f"🔻 Bottom {n}","df":pd.DataFrame([{"📍 Location":c["location"],"📋 Sheet":c["sheet"],"Row #":c["row"]+1,"Col #":c["col"]+1,"Column":c["col_header"],"Value":v} for v,c in bot])})
                    return out
        ...
        if f_uniq and sig:
            ...
            if kw:
                ...
                for cell in wc:
                    if cell["is_header"]: continue
                    if kw.lower() in cell["col_header"].lower():
                        uvals.add(cell["value"]); srows.append({"Location":cell["location"],"Sheet":cell["sheet"],"Column":cell["col_header"],"Value":cell["value"]})
                tbl=(pd.DataFrame(srows).drop_duplicates(subset=["Location","Sheet","Value"]) if srows else pd.DataFrame())
                out["answer"]=f"Found **{len(uvals)}** unique value(s) in columns matching **'{kw}'**."; out["table"]=tbl; return out
 
        # ── INTENT: Free-text entity/keyword search ────────────────
        # Scans VALUES in EVERY cell at ANY row, ANY column, ANYWHERE
        if sig:
            ...
            cell_list=[{"📍 Location":c["location"],"📋 Sheet":c["sheet"],"Row #":c["row"]+1,"Col #":c["col"]+1,"Column Header":c["col_header"],"Value":c["value"]} for c in hit_cells[:40]]
 
            loc_str=" in "+", ".join(matched_locs) if matched_locs else ""
            out["answer"]=(f"Found **{len(hit_cells):,}** cell(s) matching **'{', '.join(terms[:4])}'**{loc_str}\nacross **{len(loc_freq)}** location(s) and **{len(sh_freq)}** sheet(s).\n**{len(hit_keys):,}** unique data row(s) contain this data.")
            out["table"]=full_df if not full_df.empty else None; out["cell_hits"]=cell_list
            if len(loc_freq)>1:
                out["chart_df"]=lf_df; out["chart_cfg"]={"x":"Location","y":"Hits","title":f"Hits per location — '{', '.join(terms[:3])}'"}
            if not full_df.empty:
                for col in full_df.columns:
                    if "customer" in col.lower() or "name" in col.lower():
                        cust_df=full_df[["📍 Location","📋 Sheet",col]].drop_duplicates()
                        out["sub_tables"].append({"label":f"👤 Customer Names ({len(cust_df)} rows)","df":cust_df}); break
            num_hits=[]
            for c in hit_cells:
            ...
            if num_hits and f_topn:
                ...
                out["sub_tables"].append({"label":f"🏆 Top {n} numeric","df":pd.DataFrame([{"Location":c["location"],"Sheet":c["sheet"],"Row #":c["row"]+1,"Col":c["col_header"],"Value":v} for v,c in top])})
            return out
        ...
        out["answer"]=(
            "❓ No match found.\n\n"
            "**Try:**\n"
            "• *List all customers in Noida*  |  *Find CISCO*  |  *Find AT&T*\n"
            "• *Total subscription in Airoli*  |  *Max capacity Bangalore*  |  *Top 10 subscription*\n"
    ...
    def render_answer(res: dict):
        ...
        for si in res.get("sub_tables",[]):
            with st.expander(si["label"],expanded=True): st.dataframe(si["df"],use_container_width=True)
        if res.get("cell_hits"):
            with st.expander(f"🔬 Cell-level matches — first {len(res['cell_hits'])} shown",expanded=False):
                for ch in res["cell_hits"]:
                    st.markdown(f'<div class="cell-chip">📍 <b>{ch["📍 Location"]}</b> → Sheet: <b>{ch["📋 Sheet"]}</b> | Row <b>{ch["Row #"]}</b> Col <b>{ch["Col #"]}</b> | Header: <i>{ch["Column Header"]}</i> | Value: <b>{ch["Value"]}</b></div>',unsafe_allow_html=True)
    ...
    with ic:
        user_q=st.text_input("Ask:",placeholder="Find CISCO | List all customers in Noida | Total subscription Airoli | Max capacity | Top 10 subscription",label_visibility="collapsed",key="aisq_input")
    with bc:
    ...
    examples=[
        ["List all customers","List all customers in Noida","List all customers in Bangalore","List all customers in Chennai","List all customers in Vashi","List all customers in Kolkata"],
...
# ── Footer ─────────────────────────────────────────────────────────
st.markdown("---")
st.caption(f"Sify DC · Customer & Capacity Tracker · Data dir: `{data_dir}` · Locations: {', '.join(meta['locations']) if corpus else 'none'}")
