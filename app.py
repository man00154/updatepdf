...
# ═══════════════════════════════════════════════════════
# CSS THEME — colourful, readable, white tab content
# ═══════════════════════════════════════════════════════
st.markdown("""
<style>
...
# ═══════════════════════════════════════════════════════
# FILE UPLOAD & XLS CONVERSION (LibreOffice, NOT xlrd)
# ═══════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════
# READ SHEET (openpyxl, column-cap for safety)
# ═══════════════════════════════════════════════════════
@st.cache_data(show_spinner=False)
...
# ═══════════════════════════════════════════════════════
# CELL-LEVEL INDEXING (multi-section header map)
# ═══════════════════════════════════════════════════════
def _detect_all_header_rows(df):
    hr_set = set()
    for i in range(len(df)):
        row = df.iloc[i].astype(str).str.strip()
        fm = (row.str.len() > 0) & (~row.isin(["nan","None",""])); fv = row[fm]; nf = fv.shape[0]
        if nf < 2: continue
        lm = fm & (~row.str.match(r"^-?\d+\.?\d*[eE]?[+-]?\d*$")); nl = lm.sum(); nu = fv.nunique()
        lr = nl/max(nf,1); ur = nu/max(nf,1); vc = fv.value_counts(); nr = (vc > 1).sum()
...
def _build_cell_col_map(df):
    hr_set = _detect_all_header_rows(df); hr_maps = {}
    for hr in hr_set:
        m = {}
        ...
        hr_maps[hr] = m
    sorted_hrs = sorted(hr_set); cell_map = {}
...
@st.cache_data(show_spinner=False)
def index_sheet(df):
    cell_map, hr_set = _build_cell_col_map(df); cells = []; row_recs = {}
    for r in range(df.shape[0]):
        for c in range(df.shape[1]):
            raw = df.iat[r, c]
            ...
            ch = cell_map.get((r,c), f"Col_{c}"); is_hdr = r in hr_set
...
def build_corpus(file_list, folder):
    ...
    for fname in file_list:
        ...
        loc = location_from_name(fname); sheets = load_file(full)
...
if not excel_files:
    st.error("### ⚠️ No Excel files found\n\nCreate `excel_files/` folder or upload files via the sidebar."); st.stop()
...
if not all_sheets: st.error(f"Could not read sheets from **{selected_file}**."); st.stop()
...
with tabs[0]:
    ...
    st.markdown("---")
    st.markdown('<div class="sec-title">📄 Complete Sheet Data (all rows · all columns · all positions)</div>',unsafe_allow_html=True)
    st.caption(f"Showing all {raw_df.shape[0]} rows × {raw_df.shape[1]} columns from the raw Excel sheet.")
    st.dataframe(raw_df, use_container_width=True, height=500)
    st.download_button("⬇️ Download Complete Sheet CSV", raw_df.to_csv(index=False).encode(), f"{loc_label}_{selected_sheet}_complete.csv", "text/csv", key="ov_dl")
    ...
    if not num_cols: st.info("No numeric columns.")
        ...
        if ctype=="Bar Chart":
            ...
            if len(num_cols)>=3:
                ...
                if lc!="None": d=d.copy(); d[lc]=df_clean[lc]
            ...
            if len(num_cols)>=3:
                ...
                if cc!="None": d=d.copy(); d[cc]=df_clean[cc]
...
# ═══════════════════════════════════════════════════════
# TAB 8 – AI SMART QUERY (NO self-asking questions)
# ═══════════════════════════════════════════════════════
with tabs[8]:
    ...
    _OP_VERBS = {"total","sum","avg","mean","max","min","count","list","find","show","all","average","maximum","minimum","highest","lowest","top","bottom","describe","statistics","stats","summary","unique","distinct","sheet","column","row","missing","null","percent","percentage","ratio","share","number","across","compare"}
    _SYN = {"subscription":["subscription","subscribed","subscript"],"capacity":["capacity","capac"],"power":["power","kw","kva"],"usage":["usage","utilization","consumption","consumed"],"rack":["rack","racks"],"space":["space","sqft","sq ft"],"customer":["customer","name","customers","client","clients"],"billing":["billing","bill","invoice"],"ownership":["ownership","owned","owner"]}
    ...
    def sheet_query(question):
        ...
        f_list=any(x in ql for x in ["list","show","name","names","display"])
        out={"answer":"","table":None,"chart_df":None,"chart_cfg":None,"cell_hits":[],"sub_tables":[]}
        wc=sq_cells; rr=sq_rows
        if not wc: out["answer"]="❓ No data in this sheet."; return out
        ...
        def build_rows_df(row_nums):
            ...
            for rn in sorted(row_nums):
                rec=rr.get(rn,{})
                if rec: rd={"Row #":rn+1}; rd.update(rec); recs.append(rd)
        ...
        if (f_cust or f_list) and not f_num:
            ...
            if found:
                ...
                out["answer"]=f"Found **{len(tbl)}** customer/name entries."; out["table"]=tbl
            else: out["answer"]="No 'Customer/Name' columns found. Try: *Find CISCO* or *Show columns*"
            return out
        # INTENT: Missing
        if f_miss:
        ...
        if f_uniq:
            for w in col_kws or sig:
                ...
                for cell in wc:
                    if cell["is_header"]: continue
                    if _mcol(w,cell["col_header"]): uv.add(cell["value"]); sr.append({"Column":cell["col_header"],"Value":cell["value"],"Row #":cell["row"]+1})
                if uv:
        ...
        if sig:
            ...
            if quoted: terms=[quoted[0].lower()]
            else:
            ...
            hit_rows=sorted({c["row"] for c in hit_cells}); full_df=build_rows_df(hit_rows)
            cell_list=[{"Row #":c["row"]+1,"Col #":c["col"]+1,"Column Header":c["col_header"],"Value":c["value"]} for c in hit_cells[:100]]
            ...
            if not full_df.empty:
                for col in full_df.columns:
                    if any(x in col.lower() for x in ["customer","name","client"]):
                        ...
                        out["sub_tables"].append({"label":f"👤 Names ({len(cdf)})","df":cdf}); break
            return out
        out["answer"]="❓ No match. Try typing a customer name, keyword, or numeric query."
        return out
    ...
    # ── Input bar ONLY (no self-asking example chips) ──
    st.markdown("---")
    ic,bc,cc = st.columns([8,1,1])
    with ic: user_q=st.text_input("Ask:",placeholder="Type your query here…",label_visibility="collapsed",key="sq_input")
    ...
    if ask_btn and user_q.strip():
        with st.spinner(f"Scanning sheet…"):
